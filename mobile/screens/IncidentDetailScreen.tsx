...
export default function IncidentDetailScreen({ navigation, route }: Props) {
  const incident = getIncident(route.params.incidentId);
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Pressable style={styles.call} onPress={() => Linking.openURL(callNumber("991"))}>
          <Text style={styles.callText}>In danger right now? Call 991</Text>
...
const styles = StyleSheet.create({
  ...
  sub: { color: colors.muted, marginTop: 8, marginBottom: 18, lineHeight: 21 },
  call: {
